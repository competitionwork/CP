...
+(UINavigationController*)NavigationControllerWithGJBar
{
    ...
    NSArray* nib = [[NSBundle mainBundle] loadNibNamed:@"GJNavigation" owner:self options:nil];
    for (id content in nib)
    {
        if ([content isKindOfClass:[UINavigationController class]])
        {
            nav = (UINavigationController*)content;
            nav.navigationBar.width = MainScreenWidth;
            break;
        }
    }
    return nav;
...
{
    if (![[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
        class_addMethod([CPNavigationBar class], @selector(drawRect:), imp_implementationWithBlock(^
                                                                                                   {
                                                                                                       UIImage *img  = getimagePINGPU([UIImage imageNamed:@"title平铺"]);
                                                                                                       [[img stretchableImageWithLeftCapWidth:img.size.width/2.0 topCapHeight:img.size.height/2.0] drawInRect:CGRectMake(0, 0, MainScreenWidth, 44)];
